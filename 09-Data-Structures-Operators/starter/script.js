...
*/
//1
for (const [goal, striker] of Object.entries(game.scored)) {
  console.log(`Goal ${goal + 1}: ${striker}`);
}

//2
let sum = 0;
let oddss = Object.values(game.odds);
for (const o of oddss) {
  sum += o;
}
console.log(sum / oddss.length);

//3
for (const [k, v] of Object.entries(game.odds)) {
  let str = k === 'x' ? 'draw' : `victory ${k}`;
  console.log(`Odds of ${str}: ${v}`);
}
...
/* 
Let's continue with our football betting app! This time, we have a map with a log of the events that happened 
during the game. The values are the events themselves, and the keys are the minutes in which each event happened 
...
const camelCase = function (word) {
  ...
  console.log(
    s.slice(0, pos) +
      s.slice(pos + 1).replace(s[pos + 1], s[pos + 1].toUpperCase())
  );
};

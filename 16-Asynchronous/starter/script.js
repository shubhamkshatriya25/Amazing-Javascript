...
3. Once you have the data, take a look at it in the console to see all the attributes that you recieved about the provided location. Then, using this 
...
PART 2
6. Now it's time to use the received data to render a country. So take the relevant attribute from the geocoding API result, and plug it into the countries 
...
const whereAmI = function (lat, long) {
  fetch(`https://geocode.xyz/${lat},${long}?geoit=json`)
    .then(response => {
      if (!response.ok)
        throw new Error(`Some problem occured. Error ${response.status}`);
    ...
    .then(response => {
      if (!response.ok)
        throw new Error(`Couldn't find the country. Error ${response.status}`);
      return response.json();
    })
